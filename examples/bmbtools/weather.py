...
class GetWeatherTool(Tool):
    ...
    def invoke(self, invoke_data) -> Union[str, float, bool, Dict]:
        ...
        text_output = f"Today's weather report for {data['location']['name']} is:\n"+"".join(
            [f"{key}: {output[key]}" for key in output.keys()])
        return text_output, 0, False, {}

    def description(self) -> str:
        return "GetWeather(location), Get the weather information today at any location."


class ForcastWeatherTool(Tool):
    def __init__(self, key):
        super().__init__()
        self.key = key
    ...
    def invoke(self, invoke_data) -> Union[str, float, bool, Dict]:
        ...
        spliter = invoke_data.rfind(",")
        location = invoke_data[:spliter]
        days = invoke_data[spliter+1:]
        param = {
            "key": self.key,
            "q": location,
            "days": int(days),
        ...
        MAX_DAYS = 14
        res_completion = res_completion["forecast"]["forecastday"][int(
            days)-1 if int(days) < MAX_DAYS else MAX_DAYS-1]
        output_dict = {}
    ...
    def description(self) -> str:
        return "ForcastWeather(location, days), Forecast weather information in some days at any location."

...
class DoubanAPI:
    ...
    def get_now_playing(self) -> List[PlayingMovieInfo]:
        ...
        response = self.fetch_page(
            f"{self._endpoint}/cinema/nowplaying/beijing/")
    ...
    def get_movie_detail(self, url: str) -> str:
        ...
        for span in spans:
            if span.text == '制片国家/地区:':
                ...
                break
        Synopsis = tree.xpath(
            './/div[@class="related-info"]/div/span')[0].text.strip()
        detail = f'是一部{region}的{types}电影，由{director}导演，{actors}等人主演.\n剧情简介：{Synopsis}'
        return detail


class PrintDetailTool(Tool):
    def __init__(self):
        super().__init__()
    ...
    def invoke(self, invoke_data) -> Union[str, float, bool, Dict]:
        ...
        if link is None:
            for movie in self.douban_api.get_coming():
                if movie["title"] == filmName:
                    link = movie["link"]
                    break

        if link is None:
            for movie in self.douban_api.get_now_playing():
                if movie["title"] == filmName:
                    link = movie["link"]
                    break

        if link is None:
            return "没有找到该电影", 0, False, {}

        return "{}{}".format(filmName, self.douban_api.get_movie_detail(link)), 0, False, {}

    def description(self) -> str:
        return """PrintDetail(args), prints the details of a movie, giving its name.
...
class NowPlayingOutFilterTool(Tool):
    def __init__(self):
        super().__init__()
    ...
    def description(self) -> str:
        return """NowPlayingOutFilter(args), prints the details of the filtered [outNum] playing films now according to region, scoreSort
            ...
            Action: NowPlayingOutFilter(全部, 3, True)
            Observation: {"title":{"34":"切腹","53":"吉赛尔","31":"小森林 夏秋篇"},"score":{"34":"9.4","53":"9.2","31":"9.0"},"region":{"34":"日本","53":"西德","31":"日本"},"director":{
                "34":"小林正树","53":"Hugo Niebeling","31":"森淳一"},"actors":{"34":"仲代达矢 / 石浜朗 / 岩下志麻","53":"卡拉·弗拉奇 / 埃里克·布鲁恩 / Bruce Marks","31":"桥本爱 / 三浦贵大 / 松冈茉优"}}
            Thought: I now know the currently playing movies with the highest scores
            Final Answer: 现在上映的评分最高的3部电影是: 切腹、吉赛尔、小森林 夏秋篇
        """


class ComingOutFilterTool(Tool):
    def __init__(self):
        super().__init__()
    ...
    def invoke(self, invoke_data) -> Union[str, float, bool, Dict]:
        ...
        for i, movie in enumerate(coming_movies[:outNum]):
            ...
            ret["region"][i] = movie["region"]
            ret["wantWatchPeopleNum"][i] = "{}人".format(
                movie["wantWatchPeopleNum"])
        return ret, 0, False, {}

    def description(self) -> str:
        return """ComingOutFilter(args), prints the details of the filtered [outNum] coming films now according to region, cate and outNum.
            ...
            Action: ComingOutFilter(中国, 剧情, 2, True)
            Observation: {"date":{"23":"04月28日","50":"07月"},"title":{"23":"长空之王","50":"热烈"},"cate":{
                "23":"剧情 / 动作","50":"剧情 / 喜剧"},"region":{"23":"中国大陆","50":"中国大陆"},"wantWatchPeopleNum":{"23":"39303人","50":"26831人"}}
